...
BaroSnapshot BaroEvidence::Evaluate(int64_t tMs, bool workplaceReady, double minDescentM)
{
    ...
    if (!baseline_ready_ && workplaceReady && stable) { baseline_hpa_ = pressure; baseline_ready_ = true; }
    ...
    if (prev_pressure_hpa_ > 0.0) out.descending = std::max(0.0, std::min(1.0, (pressure - prev_pressure_hpa_) / 0.18));
